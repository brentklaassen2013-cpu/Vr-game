...
const G = {
  ...
  desktop: false, lastT: performance.now(), directorT: 0, spawnT: 0, bossSpawned: false, boss: null, ended: false, attackGrace: 0, damageFlashT: 0, musicT: 0, lastMusicBand: '', lastRank: 'D',
  objective: null, meta: JSON.parse(localStorage.getItem('crazyOfficeNightShiftMeta')||'{"level":1,"xp":0,"coins":0,"bestRank":"D","totalKOs":0,"bestCombo":0,"shifts":0}')
};
...
function musicPulse(){
  if(G.state!=='shift')return; const band=G.boss?'boss':G.chaos>72?'mayhem':G.chaos>35?'tension':'shift';
...
function haptic(ctrl,power=.4,dur=35){ try{ctrl?.inputSource?.gamepad?.hapticActuators?.[0]?.pulse(Math.min(1,power),dur);}catch{} }
...
function saveMeta(){ localStorage.setItem('crazyOfficeNightShiftMeta',JSON.stringify(G.meta)); }
...
function batColor(){ return salaryTier()==='EXECUTIVE'?'#9b63ff':salaryTier()==='MIDNIGHT'?'#24334c':salaryTier()==='CHROME'?'#b8c1ca':'#6e4128'; }
...
function buildHub(scene){
  ...
  const tip=label(scene,'SELECT A SHIFT\nwith TRIGGER',new B.Vector3(0,.72,1.7),.42,'#cfefff'); G.lobbyMeshes.push(tip); const career=label(scene,`CAREER ${G.meta.level}  •  ${salaryTier()} BAT  •  BEST ${G.meta.bestRank||'D'}`,new B.Vector3(0,2.35,2),.34,'#9aa8ff'); G.lobbyMeshes.push(career);
  updateHUD();
}

function startShift(scene,mode='SHIFT'){
  clearList(G.lobbyMeshes); G.state='shift'; G.mode=mode; G.hp=100; G.maxHp=100; G.score=0; G.chaos=0; G.combo=0; G.kills=0; G.blocks=0; G.perfectBlocks=0; G.propHits=0; G.bestCombo=0; G.wave=1; G.shiftTime=0; G.difficulty=1; G.spawnT=0; G.directorT=0; G.bossSpawned=false; G.boss=null; G.lastRank='D'; G.attackGrace=1.1; G.musicT=0; G.lastMusicBand=''; G.nextPromotion=3; G.upgrade={power:1,defense:1,improvised:1,durability:1,recovery:0}; const ct=careerTier(); G.maxHp=100+ct*3; G.hp=G.maxHp; G.upgrade.improvised+=ct*.025; G.bat={durability:100+ct*4,max:100+ct*4,broken:false,respawnT:0,crack:0}; chooseObjective();
...
function buildArena(scene){
  const theme=G.mode==='SURVIVAL'?{floor:'#20252c',wall:'#111821',accent:'#5ee0b1',name:'RECORDS FLOOR • ARCHIVE LOCKDOWN'}:G.mode==='RIOT'?{floor:'#30252a',wall:'#1a1217',accent:'#ff5d74',name:'EXECUTIVE FLOOR • BREAKROOM RIOT'}:{floor:'#252a33',wall:'#151a22',accent:'#69b7ff',name:'FLOOR 13 • PERFORMANCE REVIEW'};
...
function makeProp(scene,type,pos,opt={}){
  const s=opt.size||[.45,.45,.45], mesh=box(scene,'prop_'+type,pos,new B.Vector3(...s),type==='mug'?'#d9e2e8':type==='printer'?'#767d87':type==='chair'?'#394452':'#2f6a87');
...
class NPC{
  constructor(scene,pos,type='worker'){
    ...
    const body=box(scene,'npcBody',new B.Vector3(0,1.05,0),new B.Vector3(.48,1.05,.32),type==='boss'?'#8f223f':'#d5d7dc',this.root);
    ...
    const tie=box(scene,'npcTie',new B.Vector3(0,1.18,-.34),new B.Vector3(.12,.42,.04),type==='boss'?'#ffcf44':'#cf334e',this.root);
  ...
  dropProp(){ if(!this.prop)return; this.prop.npcOwner=null; this.prop.vel=new B.Vector3((Math.random()-.5)*2,1,(Math.random()-.5)*2); this.prop=null; }
  claimProp(){
    let best=null,bd=3.2; for(const p of G.props){ if(p.heldBy||p.npcOwner||p.mass>9)continue; const d=B.Vector3.Distance(p.mesh.getAbsolutePosition(),this.root.position); if(d<bd){best=p;bd=d;} }
...
function tick(scene){
  const now=performance.now(),dt=Math.min(.033,(now-G.lastT)/1000); G.lastT=now; const pp=playerPos(scene); if(G.state==='shift'){
...
async function setupXR(scene){
  ...
  xr.baseExperience.sessionManager.onXRSessionEnded.add(()=>{ui.boot.style.display='flex';});
  return xr;
}
...
ui.status.textContent='Reborn 1.1 Final Gauntlet ready.';
updateHUD();
